...
public class UndoWorkspaceMutationTool implements AgentTool, AgentToolPromptMetadataProvider {
    ...
    /** * Inicializa a tool de undo de mutacao do workspace. * * @param rootDirectory raiz segura do projeto atual * * @author Renato Tomaz Nati * @since 2026-05-20 */
    public UndoWorkspaceMutationTool(File rootDirectory) {
    ...
    public String getName() {
        return "desfazer_mutacao_workspace";
    }
    ...
    public String execute(String jsonParameters) {
        ...
        if (rootDirectory == null || !rootDirectory.exists() || !rootDirectory.isDirectory()) {
            return "Erro Operacional: A raiz segura do projeto esta indisponivel para undo de mutacao.";
        }
        ...
        context.setInstructionSummary(
                !isBlank(instructionSummary)
                        ? instructionSummary
                        : "Undo do ultimo batch pela tool desfazer_mutacao_workspace."
        );
        ...
        try {
            ...
            return workspaceMutationFacade.undoLastBatch(context);
        } catch (Exception e) {
            return "Falha ao desfazer mutacao do workspace: " + e.getMessage();
        }
    }
    ...
    public AgentToolPromptMetadata getPromptMetadata() {
        ...
        metadata.setOneLinePurpose("Desfazer o ultimo batch de mutacao disponivel na pilha de undo.");
        metadata.setActivityDescription("Desfaz o ultimo batch de mutacao disponivel na pilha de undo do workspace.");

        metadata.addRecommendedUseCase("Use quando houver undo disponivel e o ultimo batch precisar ser revertido.");
        metadata.addRecommendedUseCase("Use apos validar estado e historico de mutacao.");
        metadata.addRecommendedUseCase("Use quando a IA concluir que o ultimo conjunto de mudancas deve ser desfeito integralmente.");

        metadata.addGuardrail("Nao use se a pilha de undo estiver vazia.");
        metadata.addGuardrail("A ferramenta atua sobre o ultimo batch, nao sobre arquivo arbitrario.");
        metadata.addGuardrail("Consulte estado e historico antes de desfazer alteracoes sensiveis.");

        metadata.addJsonExample(
                "{\\\"action\\\":\\\"executar_ferramenta\\\",\\\"tool\\\":\\\"desfazer_mutacao_workspace\\\",\\\"parameters\\\":{},\\\"explanation\\\":\\\"Preciso desfazer o ultimo batch apos validar que o undo e seguro.\\\"}"
        );
